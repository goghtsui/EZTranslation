...
public class YoudaoTranslation {
	...
	private String getErrorMessage(){
		switch (errorCode){
			...
			case QUERY_STRING_TOO_LONG:
				return IString.ERROR_QUERY_TOO_LONG;
			case CAN_NOT_TRANSLATE:
				return "无法进行有效的翻译";
			case INVALID_LANGUAGE:
				return IString.ERROR_UNSUPPORTED_LANG;
			case INVALID_KEY:
				return IString.ERROR_INVALID_KEY;
			case NO_RESULT:
				return "无词典结果";
			default:
	...
	private String getPhonetic() {
		...
		if (us_phonetic == null && uk_phonetic == null) {
			...
			if(phonetic != null){
				result = "<span class=\"pronounce\"> 拼音：<span class=\"phonetic\">[" + basic.get(PHONETIC) + "]</span></span>";
			}
		} else {
			if (us_phonetic != null) {
				result = "<span class=\"pronounce\"> 美式：<span class=\"phonetic\">["
						+ us_phonetic + "]</span></span>";
			}
			if (uk_phonetic != null) {
				if (result == null) {
					result = "";
				}
				result = result + "<span class=\"pronounce\"> 英式：<span class=\"phonetic\">["
						+ uk_phonetic + "]</span></span>";
			}
		}
		return result;
	}
